$(document).ready(function () {
  ...
  function bindButtonByPermission() {
    if ($("#parentRole").attr("data-valkey") == "1") {
      ...
      $("#lampName").prop('disabled', true);
      $("#lampDescription").prop('disabled', true);
      $("#lampLat").prop('disabled', true);
      $("#lampLong").prop('disabled', true);
      $("#groupDevicesEdit").prop('disabled', true);
    }
  }
    ...
    fetch(ENDPOINT_URL.CONTROLLER_BY_PROJECT, {
      ...
      .then((result) => {
        ...
        if (viewModel.state == "success") {
          ...
          resetControllerDropDownModal();
          bindingControllerDropdown(viewModel.data);
          bindingControllerDropdownModal(viewModel.data);
  ...
  function bindingControllerDropdownModal(jsonController) {
      if (jsonController.length > 0) {
        //$("#groupDevicesEdit").append(
        //  $(
        //    '<option value = "0">All</option>'
        //  )
        //);
  ...
  function resetControllerDropDownModal() {
    $("#groupDevicesEdit").empty();
  }
  ...
  function bindingLampTable(data, totalPage, currentPage) {
    ...
    // ดึงค่า data-valkey และ value ของ option ที่ถูกเลือก
    //const selectedOption = $("#projectCode option:selected");
    //const selectedProjectKey = selectedOption.attr("data-valkey");
    //const selectedProjectValue = selectedOption.val();
    // แก้ไขเงื่อนไขให้ตรวจสอบตัวแปรที่ถูกต้อง
    //const isNodeLampProject =
    //  selectedProjectKey == "3" && selectedProjectValue;

    //if (isNodeLampProject) {
    //  $("#dataTable .th-status").hide(); // ซ่อน Header สถานะ
    //  $("#dataTable .th-group").show(); // แสดง Header กลุ่ม
    //} else {
    //  $("#dataTable .th-status").show(); // แสดง Header สถานะ
    //  $("#dataTable .th-group").hide(); // ซ่อน Header กลุ่ม
    //}
    ...
    if (data.length == 0) {
      ...
      $.each(data, function (_, item) {
        ...
        //  .html(badgeHtml)
        //  .appendTo(tableRow);
        let lampGroupCol = jQuery("<td></td>")
          .attr("data-title", "กลุ่ม")
           .html(item.lampGroup ? item.lampGroup:'-')
          .appendTo(tableRow);

        // สลับการแสดงผลของ Cell สถานะและกลุ่ม
        //if (isNodeLampProject) {
        //  lampStatusCol.hide();
        //  lampGroupCol.show();
        //} else {
  ...
  function bindLampViewModelToModal(viewModel) {
    ...
    $("#groupDevicesEdit").val(viewModel.controllerCode);
    $("#lampName").val(viewModel.lampName);
    $("#lampGroup").val(viewModel.lampGroup);
  ...
  $("#modal-save-button").on("click", function (e) {
      let group = $("#groupDevicesEdit").val();
    ...
    let controllerData = {
      ...
      lampGroup: $("#lampGroup").val(),
      lampDescription: $("#lampDescription").val(),
      controllerCode: $("#groupDevicesEdit").val(),
      ...
      groupCode: $("#groupDevicesEdit option:selected").html(),
    };
  ...
  const getGroupDevices = async () => {
    ...
    const inputGroup_addmodal = $("#groupDevicesEdit");
